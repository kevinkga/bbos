...
`
  ...
  createFromTemplate(templateId: string, overrides?: Partial<ArmbianConfiguration>): ArmbianConfiguration | null {
    ...
    const baseConfig: ArmbianConfiguration = {
      ...
      board: { ...template.configuration.board, ...overrides?.board },
      distribution: { ...template.configuration.distribution, ...overrides?.distribution },
  ...
  private toShellScript(config: ArmbianConfiguration): string {
    return `#!/bin/bash
